...
- требовать для новых публикаций `image_origin: real` и блокировать generated/placeholder-визуалы;
- хранить project-intro registry и автоматически добавлять строку `О проекте:` во все следующие публикации новой series;
- редактировать уже опубликованный Telegram message через отдельный GitHub Actions repair-workflow без создания дубля.
...
- `image_origin: real` — обязательная декларация для новых публикаций: реальный screenshot, фото или фактический экспорт проекта, а не AI-generated/placeholder.
...
npm test
npm run audit:published-captions
```
...
Текущий отложенный источник 500 Tower Defense исключён из автоматического
поиска, чтобы не блокировать новости других проектов. Это не удаление проекта;
условие возврата описано в [отдельной заметке](docs/deferred-project-sources.md).
...
## Аудит опубликенных подписей

`npm run audit:published-captions` работает только на чтение. Он сверяет запись
из `data/published.json` с публичным исходным патчноутом и сообщает только
агрегированные категории: ручной URL/набор меток, вероятно английский текст или
недоступный исходник. Опция `-- --details` выводит только технический ключ
публикации и номер сообщения для контролируемого исправления.
...
В исходном тексте нельзя вручную писать URL, строку `Ссылка:` или набор меток. Это намеренно блокируется проверкой до публикации, чтобы в Telegram оставались одна ссылка и один набор меток.

Исторические сообщения с уже подтверждённой неисправностью исправляются только
специальным repair-скриптом: сначала read-only проверка, затем точный набор
номеров сообщений. Обычный publisher не редактирует старые сообщения.
...
## Главный пост проекта и ссылка «О проекте»

Начиная с 0.3.14 новый проект обязан сначала опубликовать `type: intro`.
После успешной публикации uNews сохраняет Telegram URL этого сообщения в
`data/project-intros.json`. Все следующие посты той же `series` получают:

```text
Ссылка: <web_url или repo_url>
О проекте: <Telegram URL первого intro-поста>

#Project #РусскаяМетка #uNews #Sunpole
```

Старые series до введения правила не блокируются, пока для них не назначен
корректный root/intro-пост.

## Реальные изображения

Для новых публикаций с `queued_at` начиная с `2026-09-04T09:30:00Z`
обязательно:
...
`real` означает реальный screenshot работающего проекта, фотографию или
фактический экспорт результата. AI-generated, fake UI и generic placeholder
для новых постов policy блокирует.

## Редактирование опубликованных постов

Workflow `Edit published uNews post` читает
