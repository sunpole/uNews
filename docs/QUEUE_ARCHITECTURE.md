...
Внутри пакета между Telegram-запросами выдерживается не меньше 61 секунды. Это намеренно намного медленнее официального технического ограничения одного сообщения в секунду для одного чата и визуально разносит посты по разным минутам. Пакет ограничен 20 постами и занимает около 20 минут. После каждого поста состояние отправляется в `main`; если checkpoint не прошёл, пакет немедленно останавливается. Оставшаяся очередь ждёт следующего планового или ручного запуска.
