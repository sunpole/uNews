...
За один запуск публикуется до 20 готовых новостей с паузой 1,2 секунды; FIFO и порядок версий обязательны. Внутри проекта соблюдается порядок версий, между проектами — FIFO по `queued_at`. Ошибка раннего патчноута блокирует только следующие версии того же проекта. Приватные репозитории не сканируются.
...
Плановая проверка выполняется раз в четыре часа на 7-й минуте (`7 */4 * * *`). Для срочной новости запустить `Publish all project news` вручную с `dry_run=false`. Готовая очередь выгружается пакетом до 20 постов, состояние фиксируется после каждого поста.
